...
// V1.2 test convenience: bootstrap the remote D1 automatically on first use.
// This avoids requiring a terminal/CLI just to apply the initial schema.
async function ensureSchema(env) {
  ...
  await env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_punches_employee_time
    ON punches(employee_id, timestamp)`).run();
  await env.DB.prepare(`CREATE INDEX IF NOT EXISTS idx_punches_time
    ON punches(timestamp)`).run();

  for (const [id, emp] of Object.entries(EMPLOYEES)) {
    await env.DB.prepare(
      `INSERT OR IGNORE INTO employees (id, name, pin) VALUES (?, ?, ?)`
    ).bind(id, emp.name, emp.pin).run();
  }
...
async function getEmployees(env) {
  if (!dbRequired(env)) {
    return Object.entries(EMPLOYEES).map(([id, e]) => ({ id, name: e.name }));
  }
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname.startsWith("/api/")) {
      ...
      if (!allowedIp(ip)) {
        return json({ ok: false, message: "店舗ネットワークからのみ利用できます。" }, 403);
      }

      if (url.pathname === "/api/status" && request.method === "GET") {
        try {
          if (!dbRequired(env)) {
            return json({ ok: true, version: "1.2", database: false, message: "D1未接続です。" });
          }
          await ensureSchema(env);
          ...
          return json({
            ...
            version: "1.2",
            database: true,
            punchCount: Number(r?.count || 0),
            employeeCount: Number(e?.count || 0),
            schema: "ready",
          });
        } catch (e) {
          return json({ ok: false, message: "D1初期化エラー: " + e.message }, 500);
        }
      }

      if (url.pathname === "/api/employees" && request.method === "GET") {
        try {
          return json({ ok: true, employees: await getEmployees(env) });
        } catch (e) {
          return json({ ok: false, message: "スタッフ取得エラー: " + e.message }, 500);
        }
      }
      ...
      if (url.pathname === "/api/auth" && request.method === "POST") {
        try {
          ...
          if (!body?.employeeId || !/^\d{4}$/.test(body.pin || "")) {
            return json({ ok: false, message: "PINが違います。" }, 401);
          }
          const emp = await env.DB.prepare(
            "SELECT id,name FROM employees WHERE id=? AND pin=? AND active=1"
          ).bind(body.employeeId, body.pin).first();
          if (!emp) return json({ ok: false, message: "PINが違います。" }, 401);
          return json({ ok: true, employeeId: emp.id, name: emp.name });
        } catch (e) {
          return json({ ok: false, message: "認証エラー: " + e.message }, 500);
        }
      }
      ...
      if (url.pathname === "/api/punch" && request.method === "POST") {
        try {
          ...
          const body = await readJson(request);
          if (!body?.employeeId) {
            return json({ ok: false, message: "スタッフが指定されていません。" }, 400);
          }
          const emp = await env.DB.prepare(
            "SELECT id,name FROM employees WHERE id=? AND active=1"
          ).bind(body.employeeId).first();
          if (!emp) return json({ ok: false, message: "スタッフが見つかりません。" }, 400);
          const last = await env.DB.prepare(
            "SELECT type,timestamp FROM punches WHERE employee_id=? ORDER BY id DESC LIMIT 1"
          ).bind(emp.id).first();
          const type = last?.type === "in" ? "out" : "in";
          const timestamp = new Date().toISOString();
          await env.DB.prepare(
            "INSERT INTO punches (employee_id,name,type,timestamp,client_ip) VALUES (?,?,?,?,?)"
          ).bind(emp.id, emp.name, type, timestamp, ip).run();
          return json({ ok: true, type, timestamp, employeeId: emp.id });
        } catch (e) {
          return json({ ok: false, message: "打刻保存エラー: " + e.message }, 500);
        }
      }
      ...
      if (url.pathname === "/api/history" && request.method === "GET") {
        try {
          ...
          const r = employeeId
            ? await env.DB.prepare(sql).bind(employeeId, limit).all()
            : await env.DB.prepare(sql).bind(limit).all();
          return json({ ok: true, rows: r.results || [] });
        } catch (e) {
          return json({ ok: false, message: "履歴取得エラー: " + e.message }, 500);
        }
      }
